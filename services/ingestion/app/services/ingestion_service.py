...
def ingest_documents(doc_path: str):
    ...
    # IMPORTANT: This version of llama-index does NOT accept a `vector_store=`
    # kwarg on VectorStoreIndex - it is silently ignored and a default
    # in-memory SimpleVectorStore is used instead (so nothing is persisted).
    # We must wire Chroma into the StorageContext explicitly.
