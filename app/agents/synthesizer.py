...
def synthesize(results, query, memory_context: str = None, test_mode: bool = True):
    ...
    # Truncate context in test mode to speed up LLM calls
    if test_mode and len(context) > 2000:
        print(f"✂️ Truncating context from {len(context)} to 2000 chars (test mode)")
        context = context[:2000] + "\n\n[...context truncated for test mode...]"
